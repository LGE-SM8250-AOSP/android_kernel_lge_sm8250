				       ...
				       unsigned long event, void *data)
{
	...
	if (event == LGE_PANEL_EVENT_BLANK && *blank != LGE_PANEL_STATE_BLANK)
		goto out;

	if (!panel_data || !(&(panel_data->state)))
		goto out;

	blank = &(panel_data->state);
	if (event == LGE_PANEL_EVENT_POWER && *blank == LGE_PANEL_POWER_VDDIO_OFF) {
		if (!screen_on)
			goto out;
		screen_on = false;
	...
	} else if (event == LGE_PANEL_EVENT_POWER && *blank == LGE_PANEL_POWER_DSV_OFF) {
		if (!screen_on)
			goto out;
		screen_on = false;
		if (time_after(jiffies, compaction_forced_timeout) && !delayed_work_busy(&compaction_work)) {
	...
	} else if (event == LGE_PANEL_EVENT_BLANK && *blank == LGE_PANEL_STATE_UNBLANK) {
		if (screen_on)
			goto out;
		screen_on = true;
	} else if (event == LGE_PANEL_EVENT_BLANK && *blank == LGE_PANEL_STATE_LP1) {
		if (screen_on)
			goto out;
		screen_on = true;
	} else if (event == LGE_PANEL_EVENT_BLANK && *blank == LGE_PANEL_STATE_LP2) {
		if (screen_on)
			goto out;
		screen_on = true;
	}
